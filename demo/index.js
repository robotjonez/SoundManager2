...
/* global window, document, navigator, soundManager */
...
function init() {
  ...
  el = o.getElementsByTagName('dt');
  for (i = el.length; i--;) {
    if ((i + 1) % 2 === 0) {
      utils.addClass(el[i], 'alt');
    }
  }
  el = o.getElementsByTagName('dl');
  for (i = el.length; i--;) {
    if ((i + 1) % 2 === 0) {
      utils.addClass(el[i], 'alt');
    }
  ...
  if (IS_CHRISTMAS) {
    ...
    newCSS = document.body.className.split(' ');
...
function Utils() {
  ...
  this.addClass = function(o, cStr) {
    if (!o || !cStr) return; // safety net
    if (self.hasClass(o, cStr)) return;
...
function setFilter(e, sFilterPrefix) {
  var o = e ? e.target || e.srcElement : window.event.srcElement;
  ...
  if (oName === 'li') {
    ...
    if (isClear) {
      ...
      if (!navigator.userAgent.match(/msie/i)) {
        _id('filter-box').style.paddingBottom = Math.max(0, (document.documentElement.scrollTop || window.scrollY) - utils.getOffY(_id('filter-box')) - parseInt(_id('filter-box').offsetHeight, 10) - 20) + 'px';
      }
      _id('filter-box').style.display = 'block';
...
function doChristmasLights() {
  if (IS_CHRISTMAS) {
    ...
    loadScript('demo/christmas-lights/christmaslights.js', function() {
      if (window.smashInit) {
        window.setTimeout(window.smashInit, 20);
      }
    });
  }
}
...
if (window.is_home) {
    ...
    window.threeSixtyPlayer.config = {
    ...
    soundManager.onready(function() {
      ...
      items = utils.getElementsByClassName('button-exclude', 'a', _id('inline-playlist')).concat(utils.getElementsByClassName('exclude', 'a', _id('graphic-playlist')));
...
function startStuff() {
  ...
  soundManager.onready(function() {
    if (window.turntables) {
      ...
      window.turntables.config.requireCSS = 'turntable-include';
    }
  });
}
